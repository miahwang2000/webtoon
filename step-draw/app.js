(function () {
  ...
  let ramenBowlShownToppingUrls = new Set(); // 이미 그릇 위에 표시한 토핑들 (같은 토핑은 다시 안 쌓임)
  ...
  function ramenGameLoop(ts) {
    ...
    const fieldHeight = ramenGameFieldEl.clientHeight;
    const catchY = fieldHeight - 60;
    ...
    ramenFallingItems.forEach((item) => {
      ...
      if (item.y >= catchY) {
        ...
        if (dx < RAMEN_GAME_CATCH_RANGE) {
          item.done = true;
          if (item.correct) {
            ramenGameScore += RAMEN_CORRECT_SCORE;
            item.el.classList.add('caught');
            // 새로운(아직 안 받은) 토핑이면 그릇 위에 추가로 얹어서 쌓이고, 이미 받은 토핑이면 중복으로 안 쌓임
            if (!ramenBowlShownToppingUrls.has(item.url)) {
              ramenBowlShownToppingUrls.add(item.url);
              const img = document.createElement('img');
              img.src = item.url;
              ramenGameBowlToppingsEl.appendChild(img);
            }
          } else {
  ...
  function startRamenGameRound() {
    ...
    ramenBowlShownToppingUrls = new Set();
