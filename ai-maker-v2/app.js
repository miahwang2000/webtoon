...
/* ---------------------------------------------------------
   브러시 커서: 펜/지우개의 실제 굵기를 원으로 보여준다.
   컨테이너(위치 기준) 안에 커서 div를 만들어 pointermove를 따라다니게 한다.
   --------------------------------------------------------- */
function createBrushCursor(containerEl, cursorEl) {
  function setDiameterPx(canvasEl, lineWidth) {
    const rect = canvasEl.getBoundingClientRect();
    const scale = rect.width / canvasEl.width;
    const size = Math.max(6, lineWidth * scale);
  ...
  function show() { cursorEl.style.display = 'block'; }
  function hide() { cursorEl.style.display = 'none'; }
  return { setDiameterPx, moveTo, show, hide };
}
...
/* ===========================================================
   ...
   =========================================================== */
function createDrawingController({ canvas, penBtn, eraserBtn, clearBtn, undoBtn, transparentBg, brushCursor, isPanActive }) {
  ...
  function setTool(t) {
    ...
    if (brushCursor) brushCursor.setDiameterPx(canvas, t === 'eraser' ? 30 : 4);
  }
  ...
  if (brushCursor) {
    canvas.addEventListener('pointermove', e => {
      if (panActive()) { brushCursor.hide(); return; }
      brushCursor.moveTo(e.clientX, e.clientY);
      brushCursor.show();
    });
    canvas.addEventListener('pointerenter', () => { if (!panActive()) brushCursor.show(); });
    canvas.addEventListener('pointerleave', () => brushCursor.hide());
  }
...
function initSketchStep() {
  ...
  const cursorEl = document.getElementById('sketchBrushCursor');
  const container = document.getElementById('viewport-sketch');
  const brushCursor = createBrushCursor(container, cursorEl);

  const panCtl = setupPan(container, document.getElementById('btn-pan-sketch'), {
    onToggle(on) { if (on) brushCursor.hide(); }
  });
  sketchPanCtl = panCtl;
  ...
  setupZoom(document.getElementById('zoomTarget-sketch'), {
    zoomInBtn: document.getElementById('zoom-in-sketch'),
    zoomOutBtn: document.getElementById('zoom-out-sketch'),
    zoomResetBtn: document.getElementById('zoom-reset-sketch')
  });
...
function initColorStep(paletteApi) {
  ...
  const cursorEl = document.getElementById('colorBrushCursor');
  ...
  setupZoom(document.getElementById('zoomTarget-color'), {
    zoomInBtn: document.getElementById('zoom-in-color'),
    zoomOutBtn: document.getElementById('zoom-out-color'),
    zoomResetBtn: document.getElementById('zoom-reset-color')
  });
  ...
  function setMode(m) {
    ...
    brushCursor.setDiameterPx(canvas, m === 'pen' ? 4 : 20);
  ...
  // 브러시 커서: 펜(선 잇기) 모드일 때만 실제 굵기를 원으로 보여준다.
  // 채우기 모드는 굵기 개념이 없으므로 커서를 표시하지 않는다.
  canvas.addEventListener('pointermove', e => {
    if (mode !== 'pen' || panCtl.isPanMode()) { brushCursor.hide(); return; }
    brushCursor.moveTo(e.clientX, e.clientY);
    brushCursor.show();
  });
  canvas.addEventListener('pointerleave', () => brushCursor.hide());
  ...
  document.getElementById('btn-save-color').addEventListener('click', () => {
    ...
    showToast(saved ? '💾 로컬에 저장되었어요' : '⚠️ 로컬 저장에 실패했어요');
    setupSheetStep();
    goToStep(3);
  });
}
