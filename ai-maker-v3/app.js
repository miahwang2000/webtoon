...
/* ---------------------------------------------------------
   ...
   이전에는 잉크 마스크에 모폴로지 닫힘(팽창 후 침식)을 걸었는데, 이
   방식은 눈동자처럼 원래 폭이 좁은 작은 영역까지 통째로 "벽"으로
   먹혀버려서 그 부분이 아예 채색되지 않는 부작용이 있었다.

   대신 "원본 기준으로 캔버스 테두리(바깥)에서 도달 가능한 배경 영역"과
   "잉크를 살짝 부풀린 상태에서 테두리로부터 도달 가능한 배경 영역"을
   비교해서, 살짝만 부풀려도 더 이상 도달하지 못하게 되는 자리(=실제로
   바깥과 새고 있던 좁은 통로)만 정확히 찾아 막는다. 이미 완전히 닫혀
   있는 눈/작은 무늬처럼 애초에 바깥과 연결된 적이 없는 영역은 이 계산에
   전혀 영향을 받지 않는다.
   --------------------------------------------------------- */
...
const FILL_GAP_BRIDGE_RADIUS = 4;

/** 캔버스의 현재 상태에서 "실제로 바깥과 새고 있는 좁은 통로"만 찾아
 *  막은 가상 벽 마스크를 만든다. 색칠을 시작하기 전, 선화 상태 그대로일
 *  때 한 번만 계산해서 캐시해두고 재사용한다(색칠 중간에 다시 계산할
 *  필요 없음). */
function buildFillWallMask(ctx, w, h) {
  ...
  // 원래는 바깥이었는데, 잉크를 살짝 부풀리기만 해도 더 이상 바깥이 아니게
  // 된 자리 = 진짜 끊어진 틈을 통해 새고 있던 통로. 이 통로와 원본 잉크
  // 자체만 벽으로 삼는다 (그 외의 배경/작은 닫힌 영역은 전혀 건드리지 않음).
  const wall = new Uint8Array(w * h);
  for (let i = 0; i < w * h; i++) {
    wall[i] = (inkMask[i] || (outsideRaw[i] && !outsideDilated[i])) ? 1 : 0;
  }
  return wall;
}
...
function setupColorStep() {
  ...
  loadImage(state.lineArtDataURL).then(img => {
    ...
    // 색칠이 시작되기 전, 선화만 있는 상태에서 채우기용 벽 마스크를 미리 계산해둔다
    fillWallMask = buildFillWallMask(ctx, canvas.width, canvas.height);
  });
}
