...
const ArDraw = (() => {
  const RES = 720; // fixed logical resolution, CSS scales it responsively
  ...
  function init({ guideSrc, onStrokeEndCb }) {
    ...
    guideCanvas.width = drawCanvas.width = RES;
    guideCanvas.height = drawCanvas.height = RES;
    ...
    const img = new Image();
    img.onload = () => gctx.drawImage(img, 0, 0, RES, RES);
    img.src = guideSrc;
  ...
  function loadDrawingFromDataURL(dataURL) {
    if (!dataURL) return;
    const img = new Image();
    img.onload = () => dctx.drawImage(img, 0, 0, RES, RES);
    img.src = dataURL;
  }
  ...
  function getPos(evt) {
    ...
    const x = (point.clientX - rect.left) * (RES / rect.width);
    const y = (point.clientY - rect.top) * (RES / rect.height);
    return { x, y };
  }

  function pushUndo() {
    try {
      undoStack.push(dctx.getImageData(0, 0, RES, RES));
  ...
  function floodFill(startX, startY) {
    ...
    const guideData = gctx.getImageData(0, 0, RES, RES).data;
    const drawImgData = dctx.getImageData(0, 0, RES, RES);
