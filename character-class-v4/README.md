# Character Sheet Maker (v2 - 색칠 기능)

웹툰 제작 전, 학생이 캐릭터를 기획하고 **직접 칠하기**를 할 수 있는 교육용 캐릭터 시트 제작 웹앱입니다.
회원가입/로그인/서버/DB/API 없이 순수 HTML·CSS·JS로만 동작하며, GitHub Pages에 그대로 올려 사용할 수 있습니다.

## v2 신기능

- **12색 칠하기**: 팔레트에서 색을 선택 → 캐릭터의 닫힌 영역을 클릭하면 그 영역이 색으로 채워집니다
- **색 발견 시스템**: 처음 사용하는 색의 톤을 발견할 때마다 팝업으로 알려줍니다 ("새로운 색 발견! 파란색 톤")
- **자동 색칠 초기화**: 헤어/의상/표정 파츠를 바꾸면 해당 영역의 색칠이 초기화됩니다
- **칠한 색 저장**: PNG 다운로드 시 칠한 색깔이 그대로 포함됩니다
