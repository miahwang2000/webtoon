(function () {
  ...
  // "선이 아님"으로 인정해서 그 안쪽으로 채우기가 넓게 퍼짐. 다만 진하고 확실한 선은 오차와
  // 무관하게 항상 경계로 지켜짐 (테두리 자체가 두꺼워지는 게 아니라, 선의 판정 기준이 바뀌는 방식)
  const FILL_TOLERANCE_STEP = 18;
  function applyFillTolerance() {
    if (!lineAlphaArr) return;
    const threshold = Math.max(20, 200 - fillTolerance * FILL_TOLERANCE_STEP);
