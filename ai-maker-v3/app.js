...
/* ---------------------------------------------------------
   ...
   새어나가지 않게 하기 위해, 잉크(어두운 픽셀) 마스크에 모폴로지
   닫힘(Closing = 팽창 후 침식) 연산을 적용한다. 팽창만 하면 선 전체가
   두꺼워져 보이는 흰 여백이 생기지만, 닫힘 연산은 작은 틈만 이어붙이고
   나머지 굵기는 원래대로 되돌려서 그런 부작용이 없다.
   --------------------------------------------------------- */
...
function erodeMaskSeparable(mask, w, h, r) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      let v = 1;
      const lo = Math.max(0, x - r), hi = Math.min(w - 1, x + r);
      for (let xx = lo; xx <= hi; xx++) { if (!mask[y * w + xx]) { v = 0; break; } }
      tmp[y * w + x] = v;
    }
  }
  const out = new Uint8Array(w * h);
  for (let x = 0; x < w; x++) {
    for (let y = 0; y < h; y++) {
      let v = 1;
      const lo = Math.max(0, y - r), hi = Math.min(h - 1, y + r);
      for (let yy = lo; yy <= hi; yy++) { if (!tmp[yy * w + x]) { v = 0; break; } }
      out[y * w + x] = v;
    }
  }
  return out;
...
/** 캔버스의 현재 상태에서 잉크(어두운 선) 마스크를 뽑아 작은 틈을 이어붙인
 *  "가상 벽" 마스크를 만든다. 색칠을 시작하기 전, 선화 상태 그대로일 때
 *  한 번만 계산해서 캐시해두고 재사용한다(색칠 중간에 다시 계산할 필요 없음). */
function buildFillWallMask(ctx, w, h) {
  ...
  const dilated = dilateMaskSeparable(inkMask, w, h, FILL_GAP_BRIDGE_RADIUS);
  return erodeMaskSeparable(dilated, w, h, FILL_GAP_BRIDGE_RADIUS);
}
