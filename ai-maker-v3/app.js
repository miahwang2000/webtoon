...
function loadGuideImage(url) {
  ...
  const opacity = cfg.GUIDE_OPACITY != null ? cfg.GUIDE_OPACITY : 0.18;
...
const BUBBLE_PLACEHOLDER = '대사를 입력하세요';

function addBubble(xPct, yPct, initialText) {
  ...
  const isPlaceholder = !initialText || initialText === BUBBLE_PLACEHOLDER;
  const displayText = isPlaceholder ? BUBBLE_PLACEHOLDER : initialText;

  el.innerHTML = `
    <span class="bubble-handle">⠿</span>
    <div class="bubble-text${isPlaceholder ? ' is-placeholder' : ''}" contenteditable="true" style="display:inline;">${displayText}</div>
    <div class="bubble-remove" role="button" aria-label="말풍선 삭제">✕</div>
  `;
  layer.appendChild(el);

  // 실제로 저장되는 텍스트(record.text)는 안내문구일 때는 빈 값으로 둔다.
  // 최종 캐릭터 시트 이미지에 "대사를 입력하세요"라는 안내문구가 그대로
  // 찍혀나가지 않도록 하기 위함이다.
  const record = { id, xPct, yPct, text: isPlaceholder ? '' : initialText };
  state.bubbles.push(record);

  const textEl = el.querySelector('.bubble-text');

  textEl.addEventListener('focus', () => {
    if (textEl.classList.contains('is-placeholder')) {
      textEl.textContent = '';
  ...
  textEl.addEventListener('blur', () => {
    if (!textEl.textContent.trim()) {
      textEl.textContent = BUBBLE_PLACEHOLDER;
      textEl.classList.add('is-placeholder');
      record.text = '';
    }
  });
  textEl.addEventListener('input', () => {
    if (textEl.classList.contains('is-placeholder')) return; // 안내문구 상태에서는 기록하지 않음
    record.text = textEl.innerText;
  });
