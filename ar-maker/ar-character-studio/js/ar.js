...
const ArStage = (() => {
  ...
  async function start(characterDataURL) {
    ...
    if (!stream) {
      try {
        ...
        video.play().catch(() => { /* some browsers require a tap first; harmless if so */ });
      } catch (err) {
  ...
  async function switchCamera() {
    ...
    // release the current camera FIRST — many phones only allow one active
    // camera stream at a time, so requesting a second before releasing the
    // first silently fails on those devices
    if (stream) {
      stream.getTracks().forEach(t => t.stop());
      stream = null;
    }
    ...
    try {
      ...
      // we already released the old camera, so try to get it back rather
      // than leaving the student with a dead video feed
      try {
        ...
        console.warn('이전 카메라 복구도 실패했어요', err2);
      }
    }
  }
