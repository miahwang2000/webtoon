...
function saveLocalDraft() {
  ...
  try {
    ...
    // setItem이 에러 없이 지나가도, 일부 브라우저(시크릿 모드, 저장공간
    // 제한이 걸린 웹뷰 등)에서는 실제로는 저장이 안 되고 조용히 무시되는
    // 경우가 있다. 방금 쓴 값을 바로 다시 읽어봐서 실제로 남아있는지
    // 확인해야 "저장됐다고 나오는데 실제로는 안 된" 상황을 잡아낼 수 있다.
    const readBack = localStorage.getItem(key);
    if (readBack !== json) {
      console.warn('로컬 저장 검증 실패: 쓴 값과 읽은 값이 다릅니다.', {
        wroteLength: json.length,
        readLength: readBack ? readBack.length : null
      });
      return false;
    }
    return true;
