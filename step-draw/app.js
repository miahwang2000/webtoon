(function () {
  ...
  const state = {
    ...
    ramenToppingImage: null, // 2단계 완성본 (배경 투명)
  ...
  $('ramenFramesNextBtn').addEventListener('click', async () => {
    const filled = state.ramenToppingSlots.filter((s) => s);
    if (!filled.length) {
      ClassroomGuard.showModal('토핑을 하나 이상 골라주세요!', [{ label: '확인', primary: true }]);
      return;
    }
    ClassroomGuard.showLoading('토핑 준비 중이에요...');
    await startRamenToppingDrawing();
    ClassroomGuard.hideLoading();
  });

  // 선택된 토핑들을 "그릇 안쪽" 기준 사각형 안에 2x2로 배치한 가이드 이미지를 합성함
  function composeRamenToppingGuide() {
    ...
    const filled = state.ramenToppingSlots.map((s, i) => ({ s, i })).filter((x) => x.s);

    return new Promise((resolve) => {
      if (!filled.length) { resolve(off.toDataURL('image/png')); return; }
      let loaded = 0;
      filled.forEach(({ s, i }) => {
        const img = new Image();
        img.onload = () => {
          ...
          loaded++;
          if (loaded === filled.length) resolve(off.toDataURL('image/png'));
        };
        img.src = s.guide;
      });
    });
  ...
  async function startRamenToppingDrawing() {
    const guideDataURL = await composeRamenToppingGuide();
    const syntheticChar = { id: 'ramen-topping', name: '토핑', steps: [guideDataURL] };
  ...
  function finishRamenStage() {
    if (state.ramenStage === 'topping') {
      ...
      out.toBlob((blob) => {
        state.ramenToppingImage = URL.createObjectURL(blob);
        enterRamenBowlColoring();
      }, 'image/png');
  ...
  function showRamenCombineScreen() {
    ...
    bowlImg.onload = () => {
      ...
      toppingImg.onload = () => { ctx.drawImage(toppingImg, 0, 0, CANVAS_RES, CANVAS_RES); };
      toppingImg.src = state.ramenToppingImage;
    };
