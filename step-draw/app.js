(function () {
  ...
  const state = {
    ...
    ramenBowlImage: null,    // 3단계 완성본 (배경 흰색)
    character: null,
    stepIndex: 0,
  ...
  const RAMEN_BOWL_OPENING = { left: 0.08, top: 0.13, right: 0.86, bottom: 0.50 };
  ...
  function enterRamenBowlColoring() {
    ...
    renderPalette();
    showScreen('screen-work');
    showRamenStageActionBtn('✅ 그릇 완성! 라면 완성하기');
  }

  /* ---- 라면 단계 전용: 캔버스 위에 뜨는, 글자로 된 확실한 완성 버튼 (아이콘만으로는 헷갈릴 수 있어서)
         그림을 가릴 수 있으니 손가락으로 끌어서 옮길 수 있게 함 ---- */
  function showRamenStageActionBtn(text) {
    const group = $('ramenStageActionGroup');
    const btn = $('ramenStageActionBtn');
    btn.textContent = text;
    group.style.display = 'flex';
    // 새로 뜰 때마다 기본 위치(캔버스 위쪽 중앙)로 되돌림
    group.style.left = '50%';
    group.style.top = '14px';
    group.style.transform = 'translateX(-50%)';

    const filled = ramenFilledIndexes();
    const pos = filled.indexOf(state.ramenCurrentSlotIndex);
    const showPrev = state.ramenStage === 'topping' && pos > 0;
    $('ramenPrevToppingBtn').style.display = showPrev ? 'inline-block' : 'none';
  }
  function hideRamenStageActionBtn() {
    $('ramenStageActionGroup').style.display = 'none';
  }

  $('ramenStageActionBtn').addEventListener('click', () => {
    if (state.theme === 'ramen' && workMode === 'color') finishRamenStage();
  });

  $('ramenPrevToppingBtn').addEventListener('click', () => {
    ClassroomGuard.showModal('이전 토핑으로 돌아가면 지금 그린 내용이 사라져요. 계속할까요?', [
      { label: '취소', primary: false },
      { label: '돌아가기', primary: true, onClick: goToPrevTopping }
    ]);
  });

  function goToPrevTopping() {
    const filled = ramenFilledIndexes();
    const pos = filled.indexOf(state.ramenCurrentSlotIndex);
    if (pos > 0) {
      const prevSlotIndex = filled[pos - 1];
      state.ramenToppingResults[prevSlotIndex] = null; // 다시 그릴 것이므로 이전 완성본은 비움
      startRamenToppingDrawing(prevSlotIndex);
    }
  }

  // 버튼을 누르면 동작하고, 손잡이(⠿)를 끌면 그룹 전체가 움직임 (버튼과 드래그를 분리해서 오작동 방지)
  (function setupRamenStageActionGroupDrag() {
    const group = $('ramenStageActionGroup');
    const handle = $('ramenDragHandle');
    let dragging = false;
    ...
    handle.addEventListener('pointermove', (e) => {
      ...
      const parentRect = group.parentElement.getBoundingClientRect();
      const maxLeft = parentRect.width - group.offsetWidth;
      const maxTop = parentRect.height - group.offsetHeight;
      const newLeft = Math.min(maxLeft, Math.max(0, origLeft + dx));
      const newTop = Math.min(maxTop, Math.max(0, origTop + dy));
  ...
  function enterColoringStage() {
    ...
    if (state.theme === 'ramen' && state.ramenStage === 'topping') {
      const topping = state.ramenToppingSlots[state.ramenCurrentSlotIndex];
      showRamenStageActionBtn(`✅ ${topping ? topping.name : '토핑'} 완성! 다음으로`);
    } else {
      hideRamenStageActionBtn();
    }
  }
