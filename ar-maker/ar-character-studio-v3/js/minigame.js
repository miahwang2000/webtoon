...
const ArGame = (() => {
  ...
  const BASE_SPEED = 306;   // 340 * 0.9 — tuned 10% slower
  const MAX_SPEED = 576;    // 640 * 0.9 — tuned 10% slower
  ...
  function resetRun() {
    ...
    nextSpawnAt = 286 + Math.random() * 242; // 260/220 * 1.1 — 10% wider obstacle gaps
  ...
  function update(dt) {
    ...
    if (distanceSinceSpawn >= nextSpawnAt) {
      distanceSinceSpawn = 0;
      nextSpawnAt = 286 + Math.random() * 286; // 260/260 * 1.1 — 10% wider obstacle gaps
      spawnObstacle();
    }
  ...
  return { init, startRun, jump, stop, bestScore, startLives: START_LIVES };
})();
